...
### 이메일 실제 전송 (Resend) — Vercel 사용 시

리포트 "이메일 전송"을 실제로 동작시키려면 **환경 변수**를 설정합니다.

1. [resend.com](https://resend.com) 가입 후 API 키 발급
2. Vercel 대시보드 → 프로젝트 → **Settings → Environment Variables**
3. 추가: **Name** `RESEND_API_KEY`, **Value** Resend API 키 (예: `re_xxxx`), **Environment** Production(또는 Preview) 체크
4. (선택) 자체 도메인 발신: **Name** `RESEND_FROM`, **Value** `reports@yourdomain.com` — 미설정 시 Resend 기본 도메인 사용

설정 후 재배포하면 `/api/send-report` 호출 시 해당 이메일로 리포트 안내 메일이 발송됩니다.
